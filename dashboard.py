...
def main():
    st.set_page_config(
        page_title="Electrolyser Dashboard", page_icon="⚡", layout="wide"
    )
    ...
    if not gas_loaded:
        ...
        st.write(
            f"**Date range:** {gas_prices_df.index.min()} to {gas_prices_df.index.max()}"
        )
    ...
    with st.sidebar:
        ...
        if threshold_option == "Set Manual Threshold":
            ...
            st.info(
                f"With manual threshold of €{manual_threshold:.2f}/MWh, the system will operate when electricity price is below this value."
            )
        else:
            st.info(
                f"Gas prices from {gas_file_option} file will be automatically loaded and merged with your data if 'Delivery day' column is present."
            )
        ...
        if time_interval != 60:
            st.info(
                f"Profit calculations adjusted for {time_interval}-minute intervals"
            )
